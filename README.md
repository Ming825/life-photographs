...
- 自定义翻页相册：3D 翻页动画、缩略图、键盘方向键与手机滑动操作
- 原生 HTML / CSS / JavaScript，无需构建工具，可直接部署为静态站点（如 GitHub Pages）

Bootstrap 以本地文件形式引入（见 `assets/vendor/`），不依赖外部 CDN，离线也可正常访问。
...
js/main.js                渲染逻辑与交互（按年份分组、翻页相册、导航栏效果）
...
1. 把同一个人的照片放进 `assets/img/students/`。
...
```js
{
  name: "张三",
  photos: [
    "assets/img/students/zhangsan-1.jpg",
    "assets/img/students/zhangsan-2.jpg",
    {
      src: "assets/img/students/zhangsan-3.jpg",
      title: "毕业留影",
      date: "2026年6月",
      desc: "照片说明（可选）",
    },
...
每位同学会拥有一个独立相册，`photos` 中的第一张照片作为卡片封面。页面仍会自动按 `year` 分组展示。

### 2. 新增实验室合影

在 `js/data.js` 的 `groupPhotos` 数组中新增一个事件，每个事件都有自己的 `photos` 相册：

```js
{
  title: "2026年 · 迎新合影",
  photos: [
    "assets/img/group/2026-welcome-1.jpg",
    "assets/img/group/2026-welcome-2.jpg",
  ],
  date: "2026年9月",
  desc: "简要说明这张合影的场景。",
}
...
同理修改 `activities` 数组；每个活动条目对应一个独立相册。

所有照片在卡片与相册中都会按原始宽高比完整显示，不会裁切。
